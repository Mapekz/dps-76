...
| DmgVs* family (Hunter's, Exterminator's, Ghoul Slayer's, Assassin's, Troubleshooter's, Zealot's, Mutant Slayer's) | +50% dbm vs matching enemy types via `enemyType`/`enemyTypeAny` conditions — ACTIVE since 2026-07-15 when the Target picker's race matches: the selected race's identity is its RACE edid (GetIsRace gates, Assassin's `HumanRace`) plus the RACE record's `ActorType*` keywords (HasKeyword gates), extracted into bodyparts.json (`raceEdid`/`keywords`, filtered by the same `isEnemyKeyword` predicate that classifies the conditions) and matched in `resolve.ts` via `ResolveContext.enemyTypeIds`. No target selected → inactive. NPC_ boss rows match through their resolved RACE (boss-record keywords are never consulted — no extracted condition references one). Values ride flat itemLevel curves (1→50, 100→50) on `ActorValues` OMOD properties routed through the STAT_DamageVsPerk plumbing | ESM (extracted 2026-07-10; race keywords 2026-07-15) |
...
| Tenderizer | +0.1% dbm (0.001) per stack, manual stack input 0–1000 (cap +100%). Target-side debuff (`@/data/target-debuffs`), applied UNCONDITIONALLY — any player's card can have placed the stacks, so it never gates on the user's own equipped Tenderizer | ESM 2026-07-15: PerkTenderizer01Spell magnitude 0.1 on the percent-scaled DamageRecieved AV (legacy zzz_ rank 2/3 spells calibrate the scale at 7/10 = +7%/+10%; the orphaned script-side Tenderizer_TargetDebuff path, 1 + 0.01 × AV, agrees). The 1000-stack/+100% cap is user spec, not ESM-visible |
| Follow Through / Taking One for the Team | Both are `wholeDamage` ×(1+value) target-side damage-taken debuffs, esm-walk-confirmed exact match to the card description (10/20/30/40% per rank — no wiki-vs-ESM mismatch). Both are conditional 10s-window procs (Follow Through: after a ranged/thrown sneak hit; TOftT: on a teamed player being attacked) that aren't steady-state-computable, so each is modeled as a manual damage-multiplier input — a 0/10/20/30/40% toggle group (`followThroughPct` / `takingOneForTheTeamPct`, default 0, on the Target panel) representing the player's own assumption of the debuff's active magnitude, independent of which card rank is equipped — NOT a rank-locked lookup. Applied UNCONDITIONALLY like Tenderizer (`@/data/manual-uptime`, always visible) — any player's card can have placed the debuff on the target, so it never gates on this build's own legendary-perk selection (the perk picker stays useful for build-completeness only); composes multiplicatively with the other via `foldWholeDamage`'s existing Π(1+value) (1.0×–1.4× each, up to ~1.96× if both maxed) | ESM chase 2026-07-14: Follow Through card → `Apply Combat Hit Spell` (IsStealthed + WeaponTypeThrown/Ranged) → SPEL → MGEF (`Perk to Apply`) → hidden debuff PERK `FollowThroughDamageDebuffPerk0{1-4}` (0x005A5D6D–70), `Mod Incoming Weapon Damage` ×1.1/1.2/1.3/1.4, unconditional 10s. TOftT's visible card (0x005A59C7–CA) is a pure gate (`GetPlayerTeammateCount()>0` + `IsMemberOfAPlayerTeam()`); real magnitude lives on hidden companion `LGN_TakingOneForTheTeam_DamageIncrease_Perk` (0x005B01AE–B1, `hasCard: false`, granted to the attacker), same `Mod Incoming Weapon Damage` ×1.1/1.2/1.3/1.4 mechanism |
...
| United Ordeal | Ghoul-only perk (`GHL_UnitedOrdeal`, 3 ranks): while `playerIsGhoul` AND `teammateCount ≥ 1` both hold, +1/+2/+3 to all 7 SPECIAL stats per rank. Folds via the existing condition-aware SPECIAL fold (`derivePlayerStats`) — no engine change needed; the only fix was the extractor's `GetPlayerTeammateCount` `Greater Than Or Equal To` branch, which previously fell through to `unresolved` for anything but `Equal To` and left all 21 modifiers inert | ESM PERK `GHL_UnitedOrdeal` (0x00797E45/0x00797E61/0x00797E3C); `GetPlayerTeammateCount ≥ N` extractor branch (2026-07-15, previously unresolved, now `{ kind: 'teammateCount', count: N, orMore: true }`) |
| Public team bonuses | User-selected via a Team-section toggle (None/Casual/Exploration), NOT derived/ESM-scraped like the rest of this table — a manually-modeled unconditional SPECIAL ADD keyed off `PlayerConditions.publicTeamType` (`src/data/public-teams.ts`). Casual → +1..+4 INT, Exploration → +1..+4 END; magnitude `min(teammateCount+1, 4)` as a bond-score proxy (documented simplification: real in-game bond score builds over time and isn't otherwise modeled — team size including self stands in for it). The other 6 FO76 public-team types are out of scope (non-SPECIAL or non-damage effects) | ESM `PT_PublicTeamBonuses_Perk` (0x005B7584) grants ability spells gated on team type + bond score: Casual (TeamType 6) → spell `PT_CasualTeamBonus` (0x005B7585) → MGEF `AbFortifyIntelligence` (0x0004C938, AV Intelligence 0x000002C6), +1..+4 by bond score; Exploration (TeamType 4) → spell `PT_ExplorationTeamBonus` (0x005B7587) → MGEF `AbFortifyEndurance` (0x00169559, AV Endurance 0x000002C4), +1..+4 by bond score |
...
- **Taking One for the Team / Follow Through**: the families still extract
  ...
  cards are no longer inert — a manual damage-multiplier toggle models their
  effect directly in `assemble()` (`src/lib/loadout.ts`), applied
  unconditionally; see the manual-input table above. Same "registry entries
  are display/slotting-only" gap applies to the rest of the legendary perks.
...
- Enemy DR/ER, armor pen, range falloff, limb targeting: deferred by plan.
  (Race-gated damage — `enemyType`/`enemyTypeAny` — is NO longer deferred: it
  resolves against the Target picker's race since 2026-07-15, see the DmgVs*
  row above. The 'needsEnemyDefenses' picker badge now covers only
  armorPen-bucket / `enemyDamageResist`-curve mods.)
