...
export interface CurveTableBarrel {
  /** Relative to src/data/<mode>/curvetables/, e.g. "player/damage/index.generated.ts". */
  relativePath: string;
  source: string;
}

/**
 * Build one `index.generated.ts` barrel per CURVE_TABLE_GROUPS family: a
 * statically-import-and-reexport module keyed by tier number, so consumers can
 * index a plain object instead of using Vite-only `import.meta.glob`.
 * Deterministic — same input tier set always produces byte-identical output,
 * sorted ascending by tier (extraction re-runs must not churn the file when
 * nothing changed). Singletons (CURVE_TABLE_SINGLETONS) are single files with
 * no tier axis — they get no barrel and are unaffected.
 *
 * Groups are identified by detecting directories (the part of relativePath
 * before the final '/') that match a CURVE_TABLE_GROUPS' outSubdir. Singleton
 * directories never match (they're one-off, never repeated across >1 file).
 * Files are sorted by tierFromEdid(file.editorId) ascending within each group.
 */
export function buildCurveTableBarrels(files: ExtractedCurveTableFile[]): CurveTableBarrel[] {
  const barrels: CurveTableBarrel[] = [];

  // Identify directories that match CURVE_TABLE_GROUPS' outSubdir.
  const groupDirs = new Set(CURVE_TABLE_GROUPS.map((g) => g.outSubdir));

  // Group files by their directory.
  const byDir = new Map<string, ExtractedCurveTableFile[]>();
  for (const file of files) {
    const lastSlash = file.relativePath.lastIndexOf('/');
    const dir = lastSlash >= 0 ? file.relativePath.slice(0, lastSlash) : '';
    if (groupDirs.has(dir)) {
      if (!byDir.has(dir)) byDir.set(dir, []);
      byDir.get(dir)!.push(file);
    }
  }

  // For each group, emit a barrel.
  for (const dir of Array.from(byDir.keys()).sort()) {
    const groupFiles = byDir.get(dir)!;

    // Sort by tier number.
    const sorted = groupFiles.slice().sort((a, b) => {
      const tierA = tierFromEdid(a.editorId) ?? 0;
      const tierB = tierFromEdid(b.editorId) ?? 0;
      return tierA - tierB;
    });

    // Extract filenames without directory prefix.
    const importLines = sorted
      .map((file) => {
        const filename = file.relativePath.slice(dir.length + 1); // +1 to skip the '/'
        const tier = tierFromEdid(file.editorId)!;
        return `import tier${tier} from './${filename}';`;
      })
      .join('\n');

    const exportEntries = sorted
      .map((file) => {
        const tier = tierFromEdid(file.editorId)!;
    ...
    barrels.push({
      relativePath: `${dir}/index.generated.ts`,
      source,
    });
  }
