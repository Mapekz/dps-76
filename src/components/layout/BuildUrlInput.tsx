...
export function BuildUrlInput() {
  ...
  const runImport = React.useCallback(
    (imp: PendingImport, isGhoul: boolean) => {
      dispatch({ type: 'build/importNd', perks: imp.perks, name: imp.name, special: imp.special, isGhoul });
  ...
  return (
    <div className="min-w-0 flex-1">
      ...
      <Dialog open={pending !== null} onOpenChange={open => !open && setPending(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{pending?.conflict ? 'Invalid build link' : `Switch to ${pending?.locked === 'ghoul' ? 'Ghoul' : 'Human'}?`}</DialogTitle>
            <DialogDescription>
              {pending?.conflict
          ...
          <DialogFooter>
            ...
            {pending?.conflict ? (
              <>
                <Button type="button" variant="secondary" onClick={() => { runImport(pending, false); setPending(null); }}>
                  Import as Human
                </Button>
                <Button type="button" variant="default" onClick={() => { runImport(pending, true); setPending(null); }}>
                  Import as Ghoul
                </Button>
              </>
            ) : (
              <Button
                type="button"
                variant="default"
                onClick={() => { if (pending) runImport(pending, pending.locked === 'ghoul'); setPending(null); }}
              >
                Import
              </Button>
            )}
