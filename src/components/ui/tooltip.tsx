    ...
    <TooltipPrimitive.Portal>
      ...
      >
        <TooltipPrimitive.Popup
          ...
          className={cn(
            // max-w-64: tooltips portal to the body, escaping the grid entirely, so
            // nothing else constrains their width — Phase 3 (2026-08-10 design
            // critique) added enough real definition content that an unconstrained
            // `w-fit` tooltip could otherwise stretch arbitrarily wide.
            'bg-foreground text-background animate-in fade-in-0 zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 w-fit max-w-64 origin-(--transform-origin) rounded-none px-3 py-1.5 text-xs tracking-wide text-balance',
            className,
          )}
          {...props}
        >
