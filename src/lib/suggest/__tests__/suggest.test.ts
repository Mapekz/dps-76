...
describe('enumerateVariants', () => {
  it('emits omod alternatives per slot but never the equipped option', () => {
    ...
    expect(receiverMods.some(v => v.id.endsWith('mod_CombatRifle_Receiver_Damage-Auto'))).toBe(false);
  ...
  it('offers mutation toggles in both directions', () => {
    ...
    const withMutation = stateFrom([{ type: 'mutation/toggle', id: takes[0].action.type === 'mutation/toggle' ? takes[0].action.id : '' }], fixerState);
    const drops = enumerateVariants(withMutation, 'live').filter(v => v.group === 'mutation' && v.label.startsWith('Drop'));
...
describe('evaluateSuggestions', () => {
  it('baseline equals a direct computeScenarios of the unpatched config (drift guard)', () => {
    ...
    const direct = snapshotOf(computeScenarios(resolveLoadout(fixerState.player, fixerState.enemy, 'live')!));
